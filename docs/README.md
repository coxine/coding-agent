...
- 在 TUI 中提交自然语言编程任务。
- 流式显示模型回复和当前运行状态。
- 使用 `markdansi` 渲染 GFM，支持标题、粗体、斜体、行内代码、带语法高亮的代码块、完整换行且列边框对齐的表格、嵌套列表、引用和链接等格式。
- 展示文件读取、搜索、修改和命令执行过程。
...
- 支持取消正在执行的任务。
- 在同一次运行中继续追问或提交下一项任务。
- 启动时显示空白界面；可通过 `/session` 选择历史对话，或直接发送任务创建新对话。
- 输入 `/session` 新建或切换当前工作区的历史对话。
- 第一条 prompt 自动生成对话名称，也可用 `/rename <名称>` 自定义。
- 输入 `/status` 查看模型、工作目录、会话 ID、上下文占用和运行 metadata。
- 输入 `/compact` 手动压缩当前对话的上下文（丢弃低价值输出、折叠轨迹并提取结构化状态）。
...
| `AGENT_CONTEXT_WINDOW` | 否 | 模型上下文窗口 token 上限，默认 `128000`；用于 `/status` 容量条，并按每 token 约 4 字符推导请求裁剪预算。兼容协议通常无法自动提供 |
...
对话按工作区分别保存。再次从同一目录启动 `coding-agent` 时，程序会显示空白界面，不会自动加载任何历史对话。在聊天输入区输入：
...
新对话会在发送第一条 prompt 时自动生成展示名称。需要自定义时输入：

```text
/rename Parser cleanup
```

名称会随对话持久化，最长 80 个字符；自定义后不会再被后续 prompt 覆盖。

输入 `/status` 会打开当前对话的状态面板。Token 区只用一条 Session distribution 展示最近一次成功请求的实际输入、输出、缓存、推理 token 和剩余上下文容量；按 `Enter` 或 `Esc` 关闭。该命令只读取本地状态，不会作为消息发送给模型，也不会使用本地 tokenizer 估算。Session 累计 token 会重复包含多次请求携带的历史上下文，因此不用于上下文容量图。
...
| `Y` | 在批准面板中选择“允许一次” |
| `N` | 在批准面板中选择“拒绝” |
| `Esc` | 任务运行时暂停/继续；在面板中关闭面板、拒绝批准或取消 Agent 的问题 |
| `Ctrl+R` | 展开或折叠模型思考内容 |
| `Ctrl+C` | 运行中先取消任务；空闲时退出程序 |
| `/session` | 打开对话选择面板 |
| `/rename <名称>` | 修改当前对话的展示名称 |
| `/status` | 打开当前对话状态面板 |
| `/compact` | 手动压缩当前对话上下文 |
| `/` | 打开并过滤命令列表 |
| `↑` / `↓` | 在对话选择面板中移动 |
| `N` | 在对话选择面板中新建对话 |
| `D` | 在对话选择面板中删除选中对话（需按 `Y` 确认） |
