...
输入 `/status` 会打开当前对话的状态面板。面板显示最近一次模型请求由 API 返回的实际上下文 token、本次输出 token，以及当前 Session 的累计输入、输出、缓存和推理 token；按 `Enter` 或 `Esc` 关闭。该命令只读取本地状态，不会作为消息发送给模型，也不会使用本地 tokenizer 估算。
