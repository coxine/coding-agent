...
默认 `maxSteps=1000`。准备发起下一次模型请求前检查；达到上限后返回 `max_steps_exceeded`。
...
暂停是独立于终态的运行控制，不写入模型消息历史，也不改变 turn ID。Agent 在模型流式增量、模型请求结束、工具调用前后检查暂停门；本地命令不在执行中强制冻结，而是在当前调用完成后进入暂停。继续操作解除暂停门并从原循环位置执行。暂停期间收到取消仍立即走取消流程。
