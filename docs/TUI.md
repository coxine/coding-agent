...
用户在空闲 Composer 中执行 `/status` 后显示只读状态面板。面板展示模型、目录、Conversation ID、Core Session ID、当前上下文分布和精简 metadata；Enter 或 Esc 关闭。Token 区只显示一条 Session distribution，数据来自最近一次成功模型请求，使用堆叠颜色区分普通输入、缓存输入、普通输出和推理输出。整条以 `AGENT_CONTEXT_WINDOW` 为总量并显示剩余空间，默认容量为 128,000 token。Session 累计 token 因重复包含历史上下文，不用于容量图。Token 不在本地估算，缺失 usage 时明确提示供应商未返回。
...
- 不显示角色标签，使用浅灰色整行背景和低对比度 `›` 前缀与其他内容区分。
- 保留用户换行。
...
- 支持受控的基础 Markdown：标题、粗体、斜体、删除线、行内代码、代码块、表格、列表、引用、分隔线和链接文本。表格单元格按终端显示宽度完整换行，不使用省略号截断内容；连续中文和超长单词也会硬换行并保持列分割线对齐。
...
- 颜色不是状态的唯一载体。
- 保持正文与背景有足够对比度。
- 用户输入通过浅灰背景与前缀区分，工具输出和系统通知继续使用稳定标签；颜色不是唯一的区分方式。
